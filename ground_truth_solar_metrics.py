...
def get_sun_times(lat, lng, start_date, end_date, timezone=None, delay=0.1):
    ...
    if os.path.exists(cache_path):
        try:
            with open(cache_path, "r") as f:
                ...
                return [
                    (item["date"], item["sunrise"], item["sunset"])
    ...
    while current_date <= end_date:
        ...
        try:
            response = requests.get(
                "https://api.sunrise-sunset.org/json", params=params, timeout=10
            )
            data = response.json()
            ...
            if data.get("status") == "OK":
                ...
                sunrise_hour = (
                    sunrise_dt.hour + sunrise_dt.minute / 60 + sunrise_dt.second / 3600
                )
                sunset_hour = (
                    sunset_dt.hour + sunset_dt.minute / 60 + sunset_dt.second / 3600
                )

                results.append(
                    (current_date.strftime("%Y-%m-%d"), sunrise_hour, sunset_hour)
                )
    ...
    cache_data = [
        ...
        for date, sunrise, sunset in results
    ]
    try:
