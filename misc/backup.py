        ...
        for pattern in self.patterns:
            ...
            if pattern.endswith("/"):
                ...
                if fnmatch.fnmatch(rel_path, pattern) or fnmatch.fnmatch(
                    os.path.dirname(rel_path), pattern
                ):
                    return not negate
            else:
                if (
                    fnmatch.fnmatch(rel_path, pattern)
                    or fnmatch.fnmatch(os.path.basename(rel_path), pattern)
...
def copy_with_gitignore(src_dir, dest_dir):
    ...
    print(
        f"Repository {os.path.basename(src_dir)}: {copied_count} files copied, {ignored_count} files/dirs ignored"
    )
