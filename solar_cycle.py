...
def store_sunlight_analysis_hdf5(timestamps, results, template_name, output_file):
   # Convert timestamps to Unix timestamps (floats)
    ...
    with h5py.File(output_file, 'w') as f:
       ...
       f.create_dataset('avg_brightness', data=np.array([r['avg_brightness'] for r in results]))
       f.create_dataset('sky_brightness', data=np.array([r['sky_brightness'] for r in results]))
       f.create_dataset('hsv_brightness', data=np.array([r['hsv_brightness'] for r in results]))
       f.create_dataset('bright_ratio', data=np.array([r['bright_ratio'] for r in results]))
       f.create_dataset('median_intensity', data=np.array([r['median_intensity'] for r in results]))
       f.create_dataset('mean_intensity', data=np.array([r['mean_intensity'] for r in results]))
